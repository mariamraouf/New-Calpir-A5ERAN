...
const Assessment = () => {
  ...
  const calculateResults = () => {
    const rawHours = Number(answers.hours) || 10;
    ...
    if (answers.website === 'none' || answers.website === 'outdated') {
      suggestions.push({
        ...
        desc: "You need a digital foundation that doesn't just look good but actually converts visitors into revenue."
      });
    }
    if (answers.crm === 'manual' || answers.crm === 'none') {
      suggestions.push({
        title: "Automated CRM Pipeline",
        desc: "Stop losing leads. We'll set up a system that captures every inquiry and automates the follow up process."
      });
    }
    if (rawHours > 10) {
      suggestions.push({
        title: "Custom Workflow Automation",
        desc: `We can eliminate those ${rawHours} hours of manual work by connecting your tools into a seamless engine.`
      });
    }
    if (answers.bottleneck === 'ai' || answers.stage === 'established') {
      suggestions.push({
        title: "Autonomous AI Agents",
        desc: "Deploy 24/7 agents to handle support and lead qualification while you sleep."
      });
    }
    if (suggestions.length === 0) {
      suggestions.push({
        title: "Full Ecosystem Acceleration",
        desc: "Upgrade your operations with continuous automated pipelines and high-velocity conversion optimization."
      });
    }
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-16">
              <div className="text-center">
                ...
                <p className="text-xl mono text-zinc-300">
                  We've analyzed your inputs. Here is your path to a scalable, automated business.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="border border-emerald-500/40 p-10 bg-emerald-950/30">
                  ...
                  <p className="mono text-sm text-zinc-300 leading-relaxed">
                    Your key priority is solving {String(answers.bottleneck || 'operational drag')}. Deploying a unified workflow eliminates bottlenecks and frees up founder bandwidth.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-emerald-500 text-black p-12 md:p-16 text-center">
                <h3 className="text-3xl md:text-5xl font-black uppercase tracking-tight mb-6">
                  Ready to Fix Your <br /> Infrastructure?
                </h3>
